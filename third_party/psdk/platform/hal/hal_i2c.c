...
#define I2C_DEVICE_RESET_GPIO_NUM      (516)
...
static void HalI2c_ResetDevice(void)
{
    ...
    sprintf(systemCmd, "echo out > /sys/class/gpio/gpio516/direction");
    ret = system(systemCmd);
    if (ret != 0) {
    ...
    sprintf(systemCmd, "echo 0 > /sys/class/gpio/gpio516/value");
    ret = system(systemCmd);
    if (ret != 0) {
    ...
    sprintf(systemCmd, "echo 1 > /sys/class/gpio/gpio516/value");
    ret = system(systemCmd);
    if (ret != 0) {
        printf("Set gpio value failed, %d\r\n", ret);
    }
